...
def _build_pipeline(
    ...
    bias = -_torch.tensor(mean_val, dtype=_torch.float32) @ _torch.tensor(
        components.T, dtype=_torch.float32
    )
...
def run_experiment(args):
    ...
    if args.dataset == "mnist":
        ...
        X_train, X_test, y_train, y_test, n_total = load_cifar10_binary_airplane_frog(
    ...
    for n_comp in args.n_components:
        ...
        if args.spca_mode == "minibatch":
            ...
            spca = SparsePCA(
                n_components=n_comp, random_state=29, max_iter=200, alpha=1
            )

        # Try load PCA
        pca_path = model_path(
            args.models_dir, args.dataset, "pca", n_comp, num_classes, input_dim
        )
        ...
        if loaded and args.save_models:
            ...
            clf, comps, mean_v, scaler = _build_pipeline(
                pca,
                X_train,
                y_train,
                device,
        ...
        if loaded and args.save_models:
            ...
            clf, comps, mean_v, scaler = _build_pipeline(
                ...
                device,
                num_classes,
                args.epochs * (2 if args.dataset.startswith("cifar") else 1),
                input_dim,
                image_input_shape=image_shape,
            )
    ...
    for attack in attacks:
        ...
        for n_comp in classifiers_pca.keys():
            ...
            for eps in tqdm(eps_list, desc=f"[{attack}] n_comp={n_comp}"):
                attack_params = {
                    "norm": args.norm,
                    "batch_size": getattr(args, "attack_batch_size", 256),
                }
                if attack.lower() in {"pgd", "mim"}:
                    attack_params["eps_step_ratio"] = (
                        0.25 if attack.lower() == "pgd" else 0.10
                    )
                    attack_params["max_iter"] = 40 if attack.lower() == "pgd" else 10
                if attack.lower() in {"square", "squareattack"}:
                    attack_params["max_iter"] = int(
                        getattr(args, "square_max_iter", 250)
                    )
                    attack_params["p_init"] = 0.8
                    attack_params["nb_restarts"] = int(
                        getattr(args, "square_restarts", 1)
                    )

                attack_pca = make_attack(
                    classifiers_pca[n_comp], attack, float(eps), attack_params
                )
                attack_spca = make_attack(
                    classifiers_spca[n_comp], attack, float(eps), attack_params
                )
                ...
                _, pca_adv_acc = evaluate_robustness(
                    X_clean, X_adv_pca, y_test_use, classifiers_pca[n_comp]
                )
                _, spca_adv_acc = evaluate_robustness(
                    X_clean, X_adv_spca, y_test_use, classifiers_spca[n_comp]
                )
            ...
            if args.save_samples:
                norm_label = (
                    ...
                    if args.norm == np.inf
                        ...
                        "l2"
                        if args.norm == 2
                        else ("l1" if args.norm == 1 else "custom")
                    )
                )
                if args.dataset == "mnist":
                    directory = (
        ...
        norm_label = (
            "linf"
            if args.norm == np.inf
            else ("l2" if args.norm == 2 else ("l1" if args.norm == 1 else "custom"))
        )
        plot_benchmark(
...
def main():
    p = argparse.ArgumentParser(
        description="Unified SPCA vs PCA robustness experiments"
    )
    p.add_argument("--dataset", choices=["mnist", "cifar-binary"], default="mnist")
    p.add_argument(
        "--attack",
        choices=["FGSM", "PGD", "MIM", "SQUARE"],
        default="FGSM",
        help="Single attack if --attacks is not provided",
    )
    p.add_argument(
        "--attacks",
        nargs="*",
        help="Multiple attacks to run in one go, e.g. --attacks FGSM PGD MIM SQUARE or --attacks ALL",
    )
    p.add_argument(
        "--norm",
        type=float,
        default=2,
        help="Attack norm: inf for Linf, 2 for L2, 1 for L1",
    )
    p.add_argument(
        "--n-components",
        dest="n_components",
    ...
    p.add_argument(
        "--spca-mode",
        choices=["minibatch", "sparse"],
        default="minibatch",
        help="SPCA solver mode",
    )
    p.add_argument(
        "--save-samples",
        action="store_true",
        help="Save adversarial sample panels for the first test image",
    )
    p.add_argument(
        "--save-models",
        action="store_true",
        help="Cache trained classifiers to disk and reuse if available",
    )
    p.add_argument(
        "--models-dir",
        type=str,
        default="models",
        help="Directory to store/load cached models",
    )
    # Performance knobs for attacks
    p.add_argument(
        "--attack-batch-size",
        dest="attack_batch_size",
        type=int,
        default=256,
        help="Batch size for attack generation",
    )
    p.add_argument(
        "--attack-n-test",
        dest="attack_n_test",
        type=int,
        default=None,
        help="Limit number of test examples used for attacks for speed",
    )
    p.add_argument(
        "--square-max-iter",
        dest="square_max_iter",
        type=int,
        default=250,
        help="Max iterations for SquareAttack (default reduced for speed)",
    )
    p.add_argument(
        "--square-restarts",
        dest="square_restarts",
        type=int,
        default=1,
        help="Number of restarts for SquareAttack (increase for stronger attacks)",
    )
    args = p.parse_args()
    run_experiment(args)
